/**
 * Tests for youtube-github-cross-reference Agent
 */

const { Youtube-Github-Cross-ReferenceAgent, main } = require('../main');

describe('Youtube-Github-Cross-ReferenceAgent', () => {
  let agent;

  beforeEach(() => {
    agent = new Youtube-Github-Cross-ReferenceAgent({
  ...
  describe('constructor', () => {
    ...
    test('creates agent with custom config', () => {
      const customAgent = new Youtube-Github-Cross-ReferenceAgent({
        logLevel: 'debug',
        timeout: 60000
      });
  ...
  describe('getStatus', () => {
    test('returns correct status', () => {
      ...
      expect(status.name).toBe('youtube-github-cross-reference');

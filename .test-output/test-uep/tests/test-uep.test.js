/**
 * Tests for Test UEP Agent
 */

const { TestUEPAgent, main } = require('../main');

describe('TestUEPAgent', () => {
  let agent;

  beforeEach(() => {
    agent = new TestUEPAgent({
  ...
  describe('constructor', () => {
    ...
    test('creates agent with custom config', () => {
      const customAgent = new TestUEPAgent({
        logLevel: 'debug',
        timeout: 60000
      });
  ...
  describe('getStatus', () => {
    test('returns correct status', () => {
      ...
      expect(status.name).toBe('Test UEP');

/**
 * Tests for Task Management API Agent
 */

const { TaskManagementAPIAgent, main } = require('../main');

describe('TaskManagementAPIAgent', () => {
  let agent;

  beforeEach(() => {
    agent = new TaskManagementAPIAgent({
  ...
  describe('constructor', () => {
    ...
    test('creates agent with custom config', () => {
      const customAgent = new TaskManagementAPIAgent({
        logLevel: 'debug',
        timeout: 60000
      });
  ...
  describe('getStatus', () => {
    test('returns correct status', () => {
      ...
      expect(status.name).toBe('Task Management API');

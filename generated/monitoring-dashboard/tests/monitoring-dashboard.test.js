/**
 * Tests for monitoring-dashboard Agent
 */

const { MonitoringDashboardAgent, main } = require('../main');

describe('MonitoringDashboardAgent', () => {
  let agent;

  beforeEach(() => {
    agent = new MonitoringDashboardAgent({
  ...
  describe('constructor', () => {
    ...
    test('creates agent with custom config', () => {
      const customAgent = new MonitoringDashboardAgent({
        logLevel: 'debug',
        timeout: 60000
      });
  ...
  describe('getStatus', () => {
    test('returns correct status', () => {
      ...
      expect(status.name).toBe('monitoring-dashboard');

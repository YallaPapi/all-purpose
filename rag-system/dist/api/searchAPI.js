...
exports.SemanticSearchAPI = void 0;
exports.createSemanticSearchAPI = createSemanticSearchAPI;
...
class SemanticSearchAPI {
    constructor(config = {}) {
        ...
        logger_1.apiLogger.info('Semantic Search API initialized', {
    ...
    async search(searchQuery) {
        ...
        logger_1.apiLogger.info('Performing semantic search', {
        ...
        try {
            ...
            const vectorSearchResults = await this.vectorClient.searchVectors(queryEmbedding.embedding, {
            ...
            const enhancedResults = await this.enhanceResults(rankedResults.slice(0, searchQuery.options?.maxResults || this.config.maxResults), searchQuery.query);
            ...
            logger_1.apiLogger.info('Search completed successfully', {
                query: searchQuery.query,
            ...
            logger_1.apiLogger.error('Search failed', {
                query: searchQuery.query,
     ...
     */
    async rankResults(results, searchQuery) {
    ...
    getSectionPriority(section) {
        ...
        if (lowerSection.includes('overview') || lowerSection.includes('introduction'))
            return 0.9;
        if (lowerSection.includes('pattern') || lowerSection.includes('methodology'))
            return 0.8;
        if (lowerSection.includes('example') || lowerSection.includes('usage'))
            return 0.7;
        if (lowerSection.includes('api') || lowerSection.includes('reference'))
            return 0.6;
        if (lowerSection.includes('config') || lowerSection.includes('setup'))
            return 0.5;
    ...
    async enhanceResults(results, originalQuery) {
        ...
        return results.map((result, index) => {
            ...
            return {
                ...
                contextScore: result.contextScore || result.score,
    ...
    generateSnippet(content, queryTerms) {
        ...
        if (bestPosition > 0)
            snippet = '...' + snippet;
        if (bestPosition + windowSize < content.length)
            snippet = snippet + '...';
        return snippet.trim();
    }
    /**
     ...
     */
    findHighlights(content, queryTerms) {
        const highlights = [];
 ...
 */
function createSemanticSearchAPI(config) {

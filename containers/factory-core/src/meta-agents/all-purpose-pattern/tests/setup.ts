...
(global as any).testUtils = {
  createSampleCode: (type: 'hardcoded' | 'universal' = 'hardcoded') => {
    if (type === 'hardcoded') {
      return `
...
expect.extend({
  toHaveAntiPattern(received: any, patternType: string) {
    const pass = received.antiPatterns?.some((p: any) => p.type === patternType);

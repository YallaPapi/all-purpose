 ...
 */
export class HardcodedArrayDetector extends BasePatternDetector {
  ...
  readonly supportedNodeTypes = ['VariableDeclarator', 'AssignmentExpression', 'Property'];
   ...
   */
  private shouldAnalyzeArray(name: string, elements: any[], context: DetectionContext): boolean {
    const config = this.config as HardcodedArrayDetectorConfig;

    // Check array size limits
    if (elements.length < (config.minArraySize || 2)) return false;
    if (elements.length > (config.maxArraySize || 1000)) return false;
    ...
    if (config.checkStringArraysOnly) {
      const hasNonStringElements = elements.some(el => 
        el && el.type !== 'StringLiteral' && el.type !== 'TemplateLiteral'
      );
      if (hasNonStringElements) return false;
    }
   ...
   */
  private calculateBusinessScore(name: string, elements: any[], context: DetectionContext): number {
    let score = 0;
    const config = this.config as HardcodedArrayDetectorConfig;
   ...
   */
  private analyzeArrayContent(elements: any[]): number {
    if (!elements.length) return 0;

    let businesslikeCount = 0;
    const totalStringElements = elements.filter(el => 
      el && (el.type === 'StringLiteral' || el.type === 'TemplateLiteral')
    ).length;

    if (totalStringElements === 0) return 0;
   ...
   */
  private getElementTypes(elements: any[]): string[] {
    const types = new Set<string>();
    elements.forEach(el => {
      if (el && el.type) types.add(el.type);
    });
   ...
   */
  private getMatchingBusinessTerms(name: string): string[] {
    const config = this.config as HardcodedArrayDetectorConfig;
    const businessTerms = config.businessTermPatterns || [];

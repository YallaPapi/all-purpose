 ...
 */
export class ConditionalLogicDetector extends BasePatternDetector {
  ...
  readonly supportedNodeTypes = ['IfStatement', 'SwitchStatement', 'ConditionalExpression'];
  ...
  detect(node: Node, path: NodePath, context: DetectionContext): DetectionResult[] {
    ...
    try {
      ...
      if (node.type === 'ConditionalExpression' && this.config.detectTernaryOperators) {
        ...
        if (ternaryResult) results.push(ternaryResult);
      }
    ...
    return this.createResult(
      ...
      context,
      {
        ...
        context: {
          ...
          hasDefault: cases.some((c: any) => c.test === null)
        },
        metadata: {
          confidence,
  ...
  private detectTernaryExpression(
    ...
    const businessScore = this.calculateBusinessScore(businessLogicInfo, context);
    if (businessScore < 0.6) return null; // Higher threshold for ternary
   ...
   */
  private analyzeCondition(test: any, context: DetectionContext): { variable: string; value: string; operator: string } | null {
   ...
   */
  private extractCaseValues(cases: any[]): string[] {
    return cases
   ...
   */
  private shouldAnalyzeSwitch(discriminant: string, caseValues: string[], context: DetectionContext): boolean {
    const config = this.config as ConditionalLogicDetectorConfig;
   ...
   */
  private isBusinessValue(variable: string, value: string, context: DetectionContext): boolean {
    const config = this.config as ConditionalLogicDetectorConfig;
   ...
   */
  private isBusinessVariable(variable: string, context: DetectionContext): boolean {
   ...
   */
  private calculateBusinessScore(businessInfo: { variable: string; value: string; operator: string }, context: DetectionContext): number {
    let score = 0;
   ...
   */
  private calculateSwitchBusinessScore(discriminant: string, caseValues: string[], context: DetectionContext): number {
    let score = 0;
    ...
    const config = this.config as ConditionalLogicDetectorConfig;
    const businessPatterns = config.businessValuePatterns || [];
    const businessValueCount = caseValues.filter(value => 
      businessPatterns.some(pattern => pattern.test(value))
    ).length;
   ...
   */
  private assessImpact(businessInfo: { variable: string; value: string; operator: string }, node: any): 'breaking' | 'major' | 'minor' | 'cosmetic' {
   ...
   */
  private assessConditionComplexity(test: any): 'simple' | 'moderate' | 'complex' {
    if (test.type === 'BinaryExpression') {
      return 'simple';
    }

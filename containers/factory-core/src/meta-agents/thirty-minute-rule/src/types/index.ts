...
export interface ThirtyMinuteRuleConfig {
  // Core timing configuration - NO hardcoded limits
  defaultTimeLimit?: number; // Default: 30 minutes in milliseconds
  maxTimeLimit?: number; // UNLIMITED by default
  minTimeLimit?: number; // Minimum allowed session time
  
  // Debug endpoint configuration - UNLIMITED endpoints supported
  debugEndpointPort?: number;
  debugEndpointPrefix?: string;
  autoGenerateEndpoints?: boolean;
  customEndpoints?: DebugEndpointConfig[];
  
  // Component isolation configuration - ALL project types supported  
  isolationTestingEnabled?: boolean;
  healthCheckInterval?: number;
  componentDiscoveryPatterns?: string[];
  
  // Alternative pathway configuration - UNLIMITED fallback strategies
  fallbackStrategies?: FallbackStrategyConfig[];
  enableAutoFallback?: boolean;
  fallbackTimeout?: number;
  
  // Integration configuration - Context7 and meta-agent coordination
  contextEnabled?: boolean;
  metaAgentCoordination?: boolean;
  coordinatorEndpoint?: string;
  
  // Project configuration - UNLIMITED project types and frameworks
  projectType?: string;
  framework?: string;

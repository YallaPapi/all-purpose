...
import { ProjectConfig, EnvironmentConfig, ApiIntegration, DatabaseIntegration, ServiceIntegration } from '../types/index.js';
...
export class ProjectAnalyzer {
   ...
   */
  async analyzeProject(projectDir: string): Promise<ProjectConfig> {
   ...
   */
  private async analyzePackageJson(projectDir: string, config: ProjectConfig): Promise<void> {
   ...
   * Analyze project structure and files
   */
  private async analyzeProjectStructure(projectDir: string, config: ProjectConfig): Promise<void> {
    try {
   ...
   */
  private async detectTechnologies(projectDir: string, config: ProjectConfig): Promise<void> {
    const technologies: ProjectConfig['technologies'] = {
      frontend: [],
      backend: [],
      database: [],
      deployment: [],
    ...
    try {
      ...
      if (await fs.pathExists(packageJsonPath)) {
        ...
        if (allDeps.vue) technologies.frontend!.push('Vue.js');
        if (allDeps.angular) technologies.frontend!.push('Angular');
        if (allDeps.next) technologies.frontend!.push('Next.js');
        if (allDeps.nuxt) technologies.frontend!.push('Nuxt.js');
        if (allDeps.svelte) technologies.frontend!.push('Svelte');

        // Backend frameworks
        if (allDeps.express) technologies.backend!.push('Express.js');
        if (allDeps.fastify) technologies.backend!.push('Fastify');
        if (allDeps.koa) technologies.backend!.push('Koa.js');
        if (allDeps.nestjs) technologies.backend!.push('NestJS');

        // Databases
        if (allDeps.mongoose) technologies.database!.push('MongoDB');
        if (allDeps.pg) technologies.database!.push('PostgreSQL');
        if (allDeps.mysql2) technologies.database!.push('MySQL');
        if (allDeps.sqlite3) technologies.database!.push('SQLite');
        if (allDeps.redis) technologies.database!.push('Redis');

        // Testing frameworks
        if (allDeps.jest) technologies.testing!.push('Jest');
        if (allDeps.mocha) technologies.testing!.push('Mocha');
        if (allDeps.cypress) technologies.testing!.push('Cypress');
        if (allDeps.playwright) technologies.testing!.push('Playwright');

        // Deployment/Build tools
        if (allDeps.webpack) technologies.deployment!.push('Webpack');
        if (allDeps.vite) technologies.deployment!.push('Vite');
        if (allDeps.rollup) technologies.deployment!.push('Rollup');
      }
      ...
      configFiles.forEach(file => {
        if (file === 'Dockerfile') technologies.deployment!.push('Docker');
        if (file === 'vercel.json') technologies.deployment!.push('Vercel');
        if (file === 'netlify.toml') technologies.deployment!.push('Netlify');
        if (file === '.github') technologies.deployment!.push('GitHub Actions');
        if (file === 'tailwind.config.js') technologies.frontend!.push('TailwindCSS');
        if (file === 'tsconfig.json') technologies.frontend!.push('TypeScript');
      });

      // Remove duplicates and empty arrays
      Object.keys(technologies).forEach(key => {
        const techKey = key as keyof typeof technologies;
   ...
   * Analyze environment configuration
   */
  private async analyzeEnvironments(projectDir: string, config: ProjectConfig): Promise<void> {
   ...
   */
  private async detectIntegrations(projectDir: string, config: ProjectConfig): Promise<void> {
    ...
    try {
      ...
      Object.keys(integrations).forEach(key => {
        const intKey = key as keyof typeof integrations;
   ...
   * Analyze existing documentation
   */
  private async analyzeExistingDocs(projectDir: string, config: ProjectConfig): Promise<void> {
    try {
      ...
      if (docFiles.length > 0) {
        ...
        const docStructure: Record<string, string[]> = {};
   ...
   */
  private async getGitInfo(projectDir: string): Promise<Record<string, any> | undefined> {
    try {

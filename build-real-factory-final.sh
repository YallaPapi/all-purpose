...
echo "Building FINAL Real Factory Core with actual meta-agents..."

# Create a temporary build directory
BUILD_DIR="/tmp/factory-build-final-$$"
mkdir -p "$BUILD_DIR"
...
# Start the factory-core server using npx tsx
CMD ["npx", "tsx", "src/factory-core.ts"]
EOF

# Build the Docker image
echo "Building Docker image..."
docker build -t real-factory-core:final "$BUILD_DIR"

# Clean up
rm -rf "$BUILD_DIR"

echo "Build complete! Run with: docker run -p 3002:3000 --network all-purpose_meta-agent-factory real-factory-core:final"
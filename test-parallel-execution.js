...
/**
 * Test Parallel Task Execution with Multiple Agents
 * 
 * Verifies that independent tasks are executed in parallel by multiple agents
 */
...
async function testParallelExecution() {
  ...
  try {
    ...
    // Create mock agents that can work in parallel
    const agents = [];
    for (let i = 1; i <= 3; i++) {
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      // Publish task to any available agent
...
function createMockAgent(agentId, nc) {
  ...
  const sub = nc.subscribe('agent.type.worker.task');
  let busy = false;
  
  (async () => {
    for await (const msg of sub) {
      if (busy) continue; // Skip if busy

...
exports.DebugEndpointGenerator = void 0;
const events_1 = require("events");
const fs_extra_1 = __importDefault(require("fs-extra"));
const path_1 = __importDefault(require("path"));
const glob_1 = require("glob");
const uuid_1 = require("uuid");
class DebugEndpointGenerator extends events_1.EventEmitter {
    config;
    discoveredComponents = new Map();
    generatedEndpoints = new Map();
    constructor(config) {
        super();
        this.config = {
    ...
    async generateEndpoints(input) {
        ...
        try {
            ...
            const outputDir = input?.outputDirectory || path_1.default.join(sourceDir, 'debug-endpoints');
            ...
            const endpointsGenerated = [];
            const errors = [];
            for (const component of components) {
                try {
                    const componentEndpoints = await this.generateComponentEndpoints(component, input?.endpointTypes || ['health', 'isolation', 'fallback', 'metrics']);
     ...
     */
    async discoverComponents(sourceDir, componentFilter) {
        const components = [];
     ...
     */
    async analyzeFileAsComponent(filePath, relativePath) {
        const content = await fs_extra_1.default.readFile(filePath, 'utf8');
        const fileExt = path_1.default.extname(filePath);
        const fileName = path_1.default.basename(filePath, fileExt);
     ...
     */
    async generateComponentEndpoints(component, endpointTypes) {
        const endpoints = [];
     ...
     */
    async generateServerCode(outputDir, endpoints) {
        await fs_extra_1.default.ensureDir(outputDir);
        // Generate main server file
        const serverCode = this.generateServerTemplate(endpoints);
        await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'debug-server.js'), serverCode);
        ...
        for (const [componentName, componentEndpoints] of handlersByComponent) {
            ...
            await fs_extra_1.default.writeFile(path_1.default.join(outputDir, handlerFileName), handlerCode);
        }
        // Generate package.json for debug server
        const packageJson = this.generatePackageJson();
        await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'package.json'), JSON.stringify(packageJson, null, 2));
        console.log(`📁 Debug server code generated in: ${outputDir}`);
    }
    /**
     * Generate server template
     */
    generateServerTemplate(endpoints) {
        const handlerImports = Array.from(new Set(endpoints.map(e => e.component))).map(component => {
...
`;
     ...
     */
    generateHandlerTemplate(componentName, endpoints) {
...
`;
     ...
     */
    async generateEndpointDocumentation(outputDir, endpoints, components) {
...
${Array.from(new Set(endpoints.map(e => e.component))).map(component => `- \`${component.toLowerCase().replace(/[^a-z0-9]/g, '-')}-handlers.js\``).join('\n')}
`;
        await fs_extra_1.default.writeFile(path_1.default.join(outputDir, 'README.md'), documentation);
        console.log(`📚 Documentation generated: ${path_1.default.join(outputDir, 'README.md')}`);
    }
    /**
     * Utility methods
     */
    shouldSkipFile(content, fileName) {
    ...
    detectComponentType(content, fileExt) {
        ...
        if (content.includes('Vue.component') || content.includes('<template>'))
            return 'Vue Component';
        if (content.includes('class ') && content.includes('extends'))
            return 'Class Component';
        if (content.includes('function ') && content.includes('export'))
            return 'Function Component';
        if (content.includes('app.') && content.includes('listen'))
    ...
    extractDependencies(content, fileExt) {
        const dependencies = [];
    ...
    detectFramework(content, fileExt) {
    ...
    isCriticalComponent(content, fileName) {
        ...
        return criticalPatterns.some(pattern => pattern.test(fileName) || pattern.test(content));
    }
    ...
    groupEndpointsByComponent(endpoints) {
    ...
    generateWarnings(components, endpoints) {
        const warnings = [];
...
exports.DebugEndpointGenerator = DebugEndpointGenerator;
//# sourceMappingURL=DebugEndpointGenerator.js.map
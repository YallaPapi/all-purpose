...
const { ProspectorAgent, main } = require('../main');

describe('ProspectorAgent', () => {
  let agent;

  beforeEach(() => {
    agent = new ProspectorAgent({
  ...
  describe('constructor', () => {
    ...
    test('creates agent with custom config', () => {
      const customAgent = new ProspectorAgent({
        logLevel: 'debug',
        timeout: 60000
      });
  ...
  describe('getStatus', () => {
    test('returns correct status', () => {
      ...
      expect(status.name).toBe('Prospector');

...
export class DataTransformationEngine extends EventEmitter {
     ...
     */
    async buildPipelinesForMappings(topology, mappings) {
    ...
    async buildSynchronizationPoints(topology) {
        ...
        syncPoints.push({
            ...
            syncType: 'commit',
            components: topology.componentLayout?.componentGroups?.map((g) => g.name) || [],
            timeout: 30000
        });
     ...
     */
    async buildSinglePipeline(mapping, topology) {
    ...
    async buildTransformationSteps(mapping) {
        const steps = [];
    ...
    mapLogicTypeToStepType(logicType) {
        switch (logicType) {
    ...
    determineExecutionMode(mapping, topology) {
        if (topology.dataFlowStrategy?.flowPattern === 'streaming')
            return 'streaming';
        if (topology.dataFlowStrategy?.flowPattern === 'batch')
            return 'batch';
        if (topology.performanceOptimizations?.parallelizationStrategy)
            return 'parallel';
        return 'sequential';
    }
    async buildRetryPolicy(topology) {
        return {
    ...
    async buildErrorHandlingStrategies(topology) {
        return [{
    ...
    async buildMonitoringConfiguration(topology) {
        return {
    ...
    async buildCachingStrategy(topology) {
        return {
    ...
    async buildPartitioningStrategy(topology) {
        return {
            strategyType: 'hash',
            configuration: {
    ...
    async buildLoadBalancingStrategy(topology) {
        return {
    ...
    async buildResourceAllocation(topology) {
        return {
    ...
    async optimizePipelineConnections(pipelines) {
    ...
    canFusePipelines(pipeline1, pipeline2) {
    ...
    calculateTotalResourceUsage(pipelines) {
...
//# sourceMappingURL=DataTransformationEngine.js.map
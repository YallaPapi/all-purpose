 ...
 * Helper Functions
 */
async function loadConfiguration(globalOptions) {
    let config = {};
...
async function parseJsonOption(option, optionName) {
    if (!option)
        return undefined;
    try {
...
function displayArchitectureResult(result) {
...
function displayTestResult(result) {
    ...
    if (result.detailedResults.recommendedActions.length > 0) {
        console.log(chalk.blue('\n💡 Recommended Actions:'));
        result.detailedResults.recommendedActions.forEach((action) => {
            console.log(chalk.blue(`  - ${action}`));
        });
    }
}
function displayAgentStatus(capabilities, stats) {
...
function displayResourceList(resources, type) {
    if (type === 'all' || type === 'architectures') {
        ...
        if (resources.architectures.length === 0) {
            ...
            resources.architectures.forEach((arch) => {
    ...
    if (type === 'all' || type === 'mappings') {
        ...
        if (resources.mappings.length === 0) {
            ...
            resources.mappings.forEach((mapping) => {
    ...
    if (type === 'all' || type === 'pipelines') {
        ...
        if (resources.pipelines.length === 0) {
            ...
            resources.pipelines.forEach((pipeline) => {
    ...
    if (type === 'all' || type === 'integrations') {
        ...
        if (resources.integrations.length === 0) {
            ...
            resources.integrations.forEach((integration) => {
...
async function saveArchitectureResult(result, outputDirectory) {

...
export class ParameterFlowAgent extends EventEmitter {
    ...
    transformationPipelines = new Map();
    testSuites = new Map();
    ...
    setupEventForwarding() {
        ...
        this.parameterMapper.on('mapping:complete', (data) => this.emit('mapping:generation:completed', data));
        this.transformationEngine.on('transformation:complete', (data) => this.emit('transformation:completed', data));
        this.testBuilder.on('test:complete', (data) => this.emit('testing:execution:completed', data));
        this.metaAgentCoordinator.on('coordination:status', (data) => this.emit('meta-agent:coordination', data));
    }
    ...
    async buildIntegrationComponents(components) {
        return await this.architectureBuilder.buildComponents(components);
    }
    async buildIntegrationConnections(topology) {
    ...
    async generateDeploymentConfiguration(topology, integrationCode) {
        return {
    ...
    async writeArchitectureToDisk(architecture, integrationCode) {
    ...
    async assessIntegrationCapabilities(architecture) {
        return {
    ...
    async assessDeploymentReadiness(architecture) {
        return {
...
//# sourceMappingURL=ParameterFlowAgent.js.map
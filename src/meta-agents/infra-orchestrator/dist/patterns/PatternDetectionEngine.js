...
export class PatternDetectionEngine {
        ...
        for (let i = 0; i < files.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async (file) => {
                try {
                    ...
                    return {
                        file,
                        report: this.createErrorReport(file, error)
                    };
                }
            });
     ...
     */
    generateSummaryReport(codebaseReport) {
    ...
    async findFiles(dirPath, patterns, exclusions) {
        const allFiles = [];
    ...
    createAnalysisReport(filePath, results, analysisTime) {
    ...
    createErrorReport(filePath, error) {
        return {
            filePath,
            totalPatterns: 0,
            errors: 1,
    ...
    generateCodebaseStatistics(fileReports, analysisTime) {

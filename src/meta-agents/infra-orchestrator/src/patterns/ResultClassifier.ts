...
export class ResultClassifier {
  private readonly categoryMappings = new Map<string, IssueCategory>([
    ['hardcoded-arrays', {
  ...
  classifyResults(results: DetectionResult[]): ClassificationReport {
    ...
    const fileIssues = new Map<string, { count: number; riskScore: number }>();
  ...
  generateRemediationSuggestion(result: DetectionResult): {
    ...
    return suggestions[result.ruleId as keyof typeof suggestions] || {
  ...
  private categorizeResult(result: DetectionResult): keyof ClassificationReport['byCategory'] {
  ...
  private getRiskLevel(result: DetectionResult): 'high' | 'medium' | 'low' {
    const category = this.categoryMappings.get(result.ruleId);
    if (category) {
  ...
  private calculateRiskScore(result: DetectionResult): number {
  ...
  private generateRecommendations(
    ...
    byCategory: ClassificationReport['byCategory'],
    bySeverity: ClassificationReport['bySeverity']
  ): string[] {
    ...
    if (byCategory.businessLogic > 5) {
      recommendations.push(
        'Consider implementing a business rules engine to externalize business logic and enable rapid adaptation to new markets and requirements.'
      );
    }

    if (byCategory.infrastructure > 3) {
      recommendations.push(
        'Implement configuration management and service discovery to improve deployment flexibility and reduce environment coupling.'
      );
    }

    if (bySeverity.error > 0) {
      recommendations.push(
        `Address ${bySeverity.error} critical error(s) immediately as they pose significant risks to system adaptability.`
      );
    }

    if (byCategory.userInterface > 10) {
      recommendations.push(
        'Implement internationalization (i18n) infrastructure to support multiple languages and customizable user experiences.'
      );
    }

    // Process improvements
    recommendations.push(
      'Establish code review guidelines that flag hardcoded business values and require configuration-driven alternatives.'
    );

    if (results.length > 20) {
      recommendations.push(
        'Consider implementing automated detection of anti-patterns in your CI/CD pipeline to prevent future violations.'
      );
    }

...
import { VariableDeclarator, AssignmentExpression, NumericLiteral, Identifier } from '@babel/types';
import { PatternDetector, DetectionResult } from '../types.js';

export class LimitationConstantDetector implements PatternDetector {
  public readonly id = 'limitation-constants';
  public readonly name = 'Limitation Constant Detector';
  public readonly description = 'Detects hardcoded limitation constants that suggest business constraints or arbitrary limits';
  ...
  private analyzeVariableDeclarator(path: NodePath<VariableDeclarator>, filePath: string, sourceCode: string): DetectionResult | null {
    const node = path.node;
    
    // Check if it's a numeric literal assignment
    if (node.init?.type !== 'NumericLiteral') return null;
  ...
  private analyzeAssignmentExpression(path: NodePath<AssignmentExpression>, filePath: string, sourceCode: string): DetectionResult | null {
    const node = path.node;
    
    // Check if it's a numeric literal assignment
    if (node.right.type !== 'NumericLiteral') return null;
  ...
  private calculateSuspicionScore(variableName: string, value: number): number {
    let score = 0;
  ...
  private isSuspiciousValue(value: number): boolean {
  ...
  private isTechnicalConstant(variableName: string, value: number): boolean {
  ...
  private isObviousBusinessLimitation(variableName: string): boolean {
  ...
  private generateMessage(variableName: string, value: number, type: 'variable' | 'assignment'): string {
  ...
  private generateSuggestion(variableName: string, value: number): string {

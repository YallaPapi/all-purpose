...
import type { TemplateEngineFactoryConfig, SystemGenerationRequest } from './types/index.js';

// Global agent instance
let agent: TemplateEngineFactoryAgent;

/**
 * Initialize agent with configuration
 */
async function initializeAgent(options: any = {}): Promise<TemplateEngineFactoryAgent> {
  if (agent) {
    return agent;
  }

  const config: TemplateEngineFactoryConfig = {
 ...
 */
function setupEventLogging(agent: TemplateEngineFactoryAgent): void {
...
program
  ...
  .action(async (systemName, options, command) => {
    try {
      ...
      const request: SystemGenerationRequest = {

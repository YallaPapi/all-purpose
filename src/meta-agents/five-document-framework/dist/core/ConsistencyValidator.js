...
export class ConsistencyValidator {
     ...
     */
    async validate(documentPaths, context) {
        try {
     ...
     */
    async validateCrossReferences(documents) {
        const checks = [];
     ...
     */
    findDocumentReferences(content) {
        const references = [];
     ...
     */
    async checkExternalReference(reference) {
     ...
     */
    async validateParameterMappings(documents, context) {
        const checks = [];
        // Extract parameters mentioned in each document
        const documentParameters = new Map();
     ...
     */
    shouldParameterBeInDocument(parameter, documentName) {
        const parameterDocumentMap = {
        ...
        if (!expectedParams)
            return false;
        return expectedParams.some(expected => parameter.toLowerCase().includes(expected.toLowerCase()) ||
            expected.toLowerCase().includes(parameter.toLowerCase()));
    }
    /**
     * Find parameter inconsistencies
     */
    findParameterInconsistencies(parameter, documents) {
     ...
     */
    findParameterUsages(parameter, content) {
        const usages = [];
     ...
     */
    async validateVersionConsistency(documents, context) {
        const versions = {};
        const suggestions = [];
     ...
     */
    async validateFormatConsistency(documents) {
        const checks = [];
        for (const [docName, content] of documents.entries()) {
            const issues = [];
    ...
    logConsistencyResults(check) {
        ...
        // Parameter mappings
        const problematicParams = check.parameterMappings.filter(param => param.missingFrom.length > 0 || param.inconsistencies.length > 0);

...
export class FiveDocumentFrameworkAgent extends EventEmitter {
    ...
    async generate(input) {
        ...
        try {
            ...
            if (this.config.validateConsistency && !input?.validateOnly) {
                ...
                consistencyCheck = await this.consistencyValidator.validate(documentResults.map(r => r.filePath), templateContext);
            }
     ...
     */
    createTemplateContext(projectConfig, generationConfig) {
        ...
        return {
            ...
            helpers: {
                ...
                joinArray: (arr, separator = ', ') => arr.join(separator),
                hasProperty: (obj, prop) => obj && Object.prototype.hasOwnProperty.call(obj, prop),
     ...
     */
    async backupExistingDocument(filePath) {
    ...
    registerHandlebarsHelpers() {
        ...
        // Conditional helpers
        Handlebars.registerHelper('hasProperty', function (obj, prop) {
        ...
        // Technical helpers
        Handlebars.registerHelper('versionBump', (version, type = 'patch') => {
            return semver.inc(version, type) || version;
        });
        // All-Purpose Pattern: Support for unlimited custom helpers
        Handlebars.registerHelper('getProperty', function (obj, prop) {

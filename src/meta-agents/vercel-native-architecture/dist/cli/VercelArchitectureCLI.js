...
export class VercelArchitectureCLI {
    ...
    async initializeAgent(outputDirectory) {
        if (this.agent)
            return;
        const config = {
    ...
    async handleBuildCommand(options) {
    ...
    async handleDeployCommand(options) {
    ...
    async handleCoordinateCommand(options) {
    ...
    async promptForBuildOptions(options) {
        const questions = [
            {
                type: 'input',
                name: 'name',
            ...
            },
            {
                ...
                filter: (input) => input ? input.split(',').map(d => d.trim()) : []
            },
            {
                type: 'confirm',
                name: 'optimization',
    ...
    async promptForRequiredOptions(options) {
        const questions = [];
        if (!options.name) {
            questions.push({
                type: 'input',
    ...
    async promptForInitOptions(options) {
        const questions = [
            {
                type: 'input',
                name: 'name',
    ...
    async loadDeploymentInfo(projectPath) {
...
//# sourceMappingURL=VercelArchitectureCLI.js.map
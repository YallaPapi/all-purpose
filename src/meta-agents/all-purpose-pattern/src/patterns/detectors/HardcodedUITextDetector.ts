 ...
 */
export class HardcodedUITextDetector extends BasePatternDetector {
  ...
  readonly supportedNodeTypes = ['VariableDeclarator', 'Property', 'AssignmentExpression', 'StringLiteral', 'TemplateLiteral', 'JSXText', 'JSXExpressionContainer'];
  ...
  private detectStringUIText(
    ...
    if (limitationScore < 0.7) return null; // Higher threshold for standalone strings
  ...
  private detectTemplateUIText(
    ...
    const templateText = quasis.map((q: any) => q.value.raw).join('${...}');
    ...
    if (limitationScore < 0.6) return null;
  ...
  private detectJSXExpressionUIText(
    ...
    if (!expression || expression.type !== 'StringLiteral') return null;
   ...
   */
  private extractTextFromValue(valueNode: any): { value: string; type: string } | null {
   ...
   */
  private shouldAnalyzeText(text: string, name: string, context: DetectionContext): boolean {
    const config = this.config as HardcodedUITextDetectorConfig;
    
    // Check text length limits
    if (text.length < (config.minTextLength || 10)) return false;
    if (text.length > (config.maxTextLength || 500)) return false;
   ...
   */
  private calculateLimitationScore(text: string, name: string, context: DetectionContext): number {
    let score = 0;
    const config = this.config as HardcodedUITextDetectorConfig;
   ...
   */
  private isUIVariable(name: string): boolean {
    const config = this.config as HardcodedUITextDetectorConfig;
   ...
   */
  private isLikelyUIContext(parentType: string | undefined, path: NodePath): boolean {
   ...
   */
  private containsSpecificLimitations(text: string): boolean {
   ...
   */
  private findLimitationPhrases(text: string): string[] {
    const config = this.config as HardcodedUITextDetectorConfig;
    const limitationPhrases = config.limitationPhrases || [];
   ...
   */
  private identifyUIContext(name: string): string {
    if (/error|warning|danger|alert/i.test(name)) return 'error-message';
    if (/success|complete|done/i.test(name)) return 'success-message';
    if (/info|information|notice/i.test(name)) return 'info-message';
    if (/placeholder|hint|help/i.test(name)) return 'input-hint';
    if (/title|heading|header/i.test(name)) return 'title-text';
    if (/button|link|action/i.test(name)) return 'action-text';
    if (/label|description|content/i.test(name)) return 'descriptive-text';
   ...
   */
  private assessTextImpact(text: string, name: string): 'breaking' | 'major' | 'minor' | 'cosmetic' {

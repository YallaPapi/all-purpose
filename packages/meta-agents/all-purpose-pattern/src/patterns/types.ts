...
export type AntiPatternType = 
  ...
  | 'hardcoded_endpoint'
  | 'hardcoded_text'
  | 'business_logic_restriction'
  | 'geographic_limitation'
  | 'numeric_constraint'
  | 'enum_limitation'
  | 'configuration_hardcoding'
  | string; // UNLIMITED extensibility for new pattern types
...
export interface DetectionResult {
  ...
  context: {
    ...
    exports?: boolean;
    imports?: string[];
    [key: string]: any; // UNLIMITED context data
  };
  metadata: {
    ...
    tags: string[];
    [key: string]: any; // UNLIMITED metadata
  };
}
...
export interface PatternDetectorConfig {
  ...
  [key: string]: any; // UNLIMITED configuration options
}
...
export interface FileReport {
  ...
  metrics: {
    codeQualityScore: number; // 0-100 based on patterns found
...
export abstract class BasePatternDetector implements PatternDetector {
  ...
  protected extractCode(node: Node, source: string): string {
    if (!node.start || !node.end) return '<unknown>';
  ...
  protected calculateSeverity(
    ...
    path: NodePath, 
    context: DetectionContext,
    baseSeverity: SeverityLevel = 'medium'
  ): SeverityLevel {
    ...
    if (node.type === 'ArrayExpression') {
      ...
      if (elements > 10) severity = this.increaseSeverity(severity);
      if (elements > 20) severity = this.increaseSeverity(severity);
    }

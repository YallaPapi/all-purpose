...
import { StringLiteral, JSXText, JSXAttribute, CallExpression, Property } from '@babel/types';
import { PatternDetector, DetectionResult } from '../types.js';

export class HardcodedUITextDetector implements PatternDetector {
  public readonly id = 'hardcoded-ui-text';
  public readonly name = 'Hardcoded UI Text Detector';
  public readonly description = 'Detects hardcoded UI text that should be externalized for localization and reuse';
  ...
  async detect(filePath: string, sourceCode: string): Promise<DetectionResult[]> {
    ...
    try {
      ...
      traverse(ast, {
        ...
        Property: (path: NodePath<Property>) => {
  ...
  private analyzeJSXText(path: NodePath<JSXText>, filePath: string, sourceCode: string): DetectionResult | null {
    const node = path.node;
    const text = node.value.trim();
    
    if (!text || text.length < 3) return null; // Skip whitespace and very short text
  ...
  private analyzeJSXAttribute(path: NodePath<JSXAttribute>, filePath: string, sourceCode: string): DetectionResult | null {
    const node = path.node;
    
    if (!node.value || node.value.type !== 'StringLiteral') return null;
    ...
    if (!userFacingAttributes.includes(attributeName)) return null;
  ...
  private analyzeCallExpression(path: NodePath<CallExpression>, filePath: string, sourceCode: string): DetectionResult | null {
    ...
    if (!this.alertMethods.some(method => callName.includes(method))) return null;
  ...
  private analyzeProperty(path: NodePath<Property>, filePath: string, sourceCode: string): DetectionResult | null {
    const node = path.node;
    ...
    if (!uiPropertyNames.some(name => propertyName.toLowerCase().includes(name))) return null;
  ...
  private calculateTextSuspicionScore(text: string): number {
  ...
  private generateMessage(context: string, text: string): string {
  ...
  private generateSuggestion(context: string, text: string): string {

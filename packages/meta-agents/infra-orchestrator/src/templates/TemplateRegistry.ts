...
export class TemplateRegistry {
  ...
  constructor(config: TemplateRegistryConfig = {}) {
    ...
    if (this.config.customHelpers) {
      Object.entries(this.config.customHelpers).forEach(([name, helper]) => {
        this.handlebarsInstance.registerHelper(name, helper as Handlebars.HelperDelegate);
      });
    }
   ...
   */
  async registerTemplatesFromDirectory(dirPath: string): Promise<void> {
  ...
  registerHelper(helper: HandlebarsHelper): void {
    this.handlebarsInstance.registerHelper(helper.name, helper.helper as Handlebars.HelperDelegate);
  ...
  private registerBuiltInHelpers(): void {
    ...
    this.handlebarsInstance.registerHelper('config', function(path: string, options: any) {
      const config = options.data.root.config || {};
      return path.split('.').reduce((obj: any, key: string) => obj && obj[key], config) || '';
    });

    // User input helper - access user-provided values
    this.handlebarsInstance.registerHelper('userInput', function(key: string, options: any) {
      const userInputs = options.data.root.userInputs || {};
      return userInputs[key] || '';
    });

    // Dynamic parameter helper - creates parameter references
    this.handlebarsInstance.registerHelper('dynamicParam', function(name: string, fallback?: string) {
      return `{{config.${name}}}` + (fallback ? ` || '${fallback}'` : '');
    });

    // Config path helper - generates proper config path references
    this.handlebarsInstance.registerHelper('configPath', function(path: string) {
      return `config.${path}`;
    });

    // Camel case helper
    this.handlebarsInstance.registerHelper('camelCase', function(str: string) {
      return str.replace(/-([a-z])/g, (g) => g[1].toUpperCase());
    });

    // Pascal case helper
    this.handlebarsInstance.registerHelper('pascalCase', function(str: string) {
      const camelCase = str.replace(/-([a-z])/g, (g) => g[1].toUpperCase());
      return camelCase.charAt(0).toUpperCase() + camelCase.slice(1);
    });

    // Snake case helper
    this.handlebarsInstance.registerHelper('snakeCase', function(str: string) {
      return str.replace(/([A-Z])/g, '_$1').toLowerCase().replace(/^_/, '');
    });

    // JSON stringify helper
    this.handlebarsInstance.registerHelper('json', function(context: any) {
      return JSON.stringify(context, null, 2);
    });

    // If equals helper
    this.handlebarsInstance.registerHelper('ifEquals', function(this: any, arg1: any, arg2: any, options: any) {
